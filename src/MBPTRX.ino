...
#include <hardware/clocks.h>  //attempt ro set spi to 40MHz
#include <hardware/spi.h>     // Required for direct SPI hardware configuration
...
static void error_stop(const uint32_t err_code) 
{
  ...
  delay(500); //separating error codes
...
void setup(void) {
  // run DSP on core 0
  //  Force the system clock to 300 MHz first
  set_sys_clock_khz(304000,true);
  delay(300);  // Give the PLL a moment to lock and stabilize
  ...
  if (clksys < (304000ul - 5ul) || clksys > (304000ul + 5ul)) {
  ...
  //Set adresses of fwd/ref ADC's here
  if (!fwdADC.begin(0)) {
  ...
  tft.init();
  //Set SPI clock to 40MHz
  spi_set_baudrate(spi0,40000000);
...
static uint8_t meter_blocks(uint32_t value) {
  ...
  if (value <= (uint32_t)METER_SCALE[0].value) {
    ...
    for (uint8_t i = 1u; i < METER_KNOTS; i++) {
...
static void show_spectrum(void) {
  ...
  if (!radio.zoom) {
    for (uint32_t x = 0; x < LCD_WIDTH; x++) {
      ...
      uint32_t droplet = magnitude[x * 3 + 33];       //zoom x1
      droplet = max(droplet,magnitude[x * 3 + 34]);  //zoom x1
      droplet = max(droplet,magnitude[x * 3 + 35]);  //zoom x1
      droplet = max(droplet,magnitude[x * 3 + 36]);  //zoom x1
      //uint32_t droplet = magnitude[x+352]; // ZOOM x3
      if (droplet > 31) droplet = 31;
      water[wp][x] = droplet;
   }
 } else {
    for (uint32_t x = 0; x < LCD_WIDTH; x++) {
      uint32_t droplet = magnitude[x + 392];  // ZOOM x3
      if (droplet > 31) droplet = 31;
      water[wp][x] = droplet;
   }
 }
...
enum menu_button_state_t {MENU_BUTTON_IDLE,
                           MENU_BUTTON_PRESSED,
                           MENU_WAIT_RELEASE_0,
                           MENU_WAIT_RELEASE_1};
enum menu_button_action_t {MENU_NONE,
                            MENU_TOP_SELECT,
                            MENU_OPTION_SELECT};
  ...
  if (tx) {
    ...
    if (radio.tx_enable) {
      if (adc_value_ready) {
        ...
        if (radio.mode == MODE_LSB || radio.mode == MODE_USB || radio.mode == MODE_DGL || radio.mode == MODE_DGU) {
        ...
        if (i2s.available()) {
          ...
          if (radio.cwdecode != 0) {
            if (radio.mode == MODE_CWL || radio.mode == MODE_CWU) {
              ...
              const uint8_t raw = radio.cwdecode == 1 ? CWDECODE1::morse_decode(cwsig) : CWDECODE2::morse_decode(cwsig);
...
static void ft8_qso_display(void) {
  ...
  for (int i = 0; i < ft8_qso.num_rows; i++) {
    ...
    uint16_t bg = 0;
    switch (row->state) {
      case FT8_QSO_ROW_PENDING:
        fg = LCD_DARKGREY;
        bg = LCD_BLACK;
        break;
      case FT8_QSO_ROW_CURRENT:
        fg = LCD_BLACK;
        bg = LCD_WHITE;
        break;
      case FT8_QSO_ROW_WAITING:
        fg = LCD_YELLOW;
        bg = LCD_BLACK;
        break;
      case FT8_QSO_ROW_RETRY:
        fg = LCD_PINK;
        bg = LCD_BLACK;
        break;
      case FT8_QSO_ROW_TIMEOUT:
        fg = LCD_RED;
        bg = LCD_BLACK;
        break;
      case FT8_QSO_ROW_DONE:
        fg = LCD_GREEN;
        bg = LCD_BLACK;
        break;
      default:
...
static void ft8_qso_check_rx(
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (ft8_qso.is_direct && step == 2) {
      ...
      const bool is_r_report = (token[0] == 'R' && (token[1] == '-' || token[1] == '+'));
      const bool is_end = (strcmp(token,"RR73") == 0 || strcmp(token,"RRR") == 0 || strcmp(token,"73") == 0);
      ...
      if (strcmp(token,"RR73") != 0 && strcmp(token,"RRR") != 0 && strcmp(token,"73") != 0) continue;
   }
        ...
        if (slot_ms < FT8_START_THRESHOLD) {
          ...
          if (ft8_qso.active) {
            ...
            if (row->is_tx) {
              ...
              should_tx = (row->state != FT8_QSO_ROW_DONE) && (ft8_selected.respond_in_even_slot == even_slot);
           }
         } else if (ft8_cq.active) {
            should_tx = (ft8_cq.even_slot == even_slot);
         } else {
            should_tx = ft8_selected.valid && (ft8_selected.respond_in_even_slot == even_slot);
         }
          if (should_tx) {
    ...
    case FT8_STATE_TRANSMITTING:
      {
        if (ft8_qso.active) {
          ...
          const int tx_step = ft8_qso.rows[ft8_qso.current_step].is_tx ? ft8_qso.current_step : ft8_qso.current_step - 1;
      ...
      case FT8_UI_BROWSE:
        {
          ...
          if (ft8_display_count > 0 && ft8_cursor >= 0 && ft8_cursor < (int32_t)ft8_display_count) {
            ft8_cq.active = false;
...
void loop(void) {
  ...
  static bool old_zoom = radio.zoom;
  ...
  enum button_state_t {BUTTON_IDLE,
                        BUTTON_TEST_SHORT,
                        BUTTON_WAIT_RELEASE};
  enum button_action_t {BUTTON_NO_PRESS,
                         BUTTON_SHORT_PRESS,
                         BUTTON_LONG_PRESS};
    ...
    if (radio.menu_active) {
      ...
      switch (option) {
        ...
        case OPTION_MODE_CWU:
          ...
          radio.mode_auto = false;
          break;
        case OPTION_MODE_DGL:
          radio.mode = MODE_DGL;
          radio.mode_auto = false;
          break;
        case OPTION_MODE_DGU:
          radio.mode = MODE_DGU;
          radio.mode_auto = false;
          break;
        case OPTION_MODE_FT8:
          radio.mode = MODE_FT8;
          radio.mode_auto = false;
          break;
        case OPTION_MODE_AM:
        ...
        case OPTION_SPEC_ZOOM_1: radio.zoom = false; break;
        case OPTION_SPEC_ZOOM_3: radio.zoom = true; break;
